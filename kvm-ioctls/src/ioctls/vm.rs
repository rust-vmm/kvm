...
impl VmFd {
    ...
    /// }
    /// ```
    pub fn enable_dirty_log_ring(&mut self, entries: Option<usize>) -> Result<bool> {
        // Check if requested number of entries is larger than 0
        if let Some(sz) = entries {
            if sz == 0 {
                return Err(errno::Error::new(libc::EINVAL));
            }
        }
        ...
        // Return EINVAL if multiplication overflows
        let cap_ring_size = entries
            .map(|e| <usize as TryInto<i32>>::try_into(e * std::mem::size_of::<kvm_dirty_gfn>()))
            .unwrap_or(Ok(max_bytes))
            .map_err(|_| errno::Error::new(libc::EINVAL))?;
